...
class uatg_decoder_illegal_instructions(IPlugin):
    ...
    def generate_asm(self):
        ...
        addi14sp = ['0b00000000000' + bin(x)[2:].zfill(3) + '00' for x in range(1, 2**3)]
        # q0 reserved: 100xxxxxxxxxxx00
        q0_reserved = ['0b100' + bin(x)[2:].zfill(11) + '00' for x in range(0, 2 ** 11)]
        reserved_insts += addi14sp + q0_reserved

        if self.isa_bit == 'rv64':
            # c.addiw: 0001x00000xxxxx01 rd/r1 = 0 -> reserved
            addiw = ['0b001' + bin(x)[2:].zfill(1) + '00000' + bin(y)[2:].zfill(5) + '01'
        ...
        lui = ['0b0110' + bin(x)[2:].zfill(5) + '0000001' for x in range(0, 2 ** 5) if
               x not in (0, 2)]

        if self.isa_bit == 'rv32':
            # c.subw: 100111xxx00xxx01 RV32 -> reserved
            subw = ['0b100111' + bin(x)[2:].zfill(3) + '00' + bin(y)[2:].zfill(3) + '01' for x in
                    range(0, 8) for y in range(0, 8)]
            # c.addw: 100111xxx01xxx01 RV32 -> reserved
            addw = ['0b100111' + bin(x)[2:].zfill(3) + '01' + bin(y)[2:].zfill(3) + '01' for x in
                    range(0, 2**3) for y in range(0, 2**3)]
            reserved_insts += addw + subw

        # q1_reserved: 100111xxx10xxx01
        #            + 100111xxx11xxx01
        q1_reserved = ['0b100111' + bin(x)[2:].zfill(3) + '10' + bin(y)[2:].zfill(3) + '01' for x
                       in range(0, 2**3) for y in range(0, 2**3)] + \
                      ['0b100111' + bin(x)[2:].zfill(3) + '11' + bin(y)[2:].zfill(3) + '01' for x
                       in range(0, 2**3) for y in range(0, 2**3)]
        reserved_insts += addi16sp + lui + q1_reserved

        # c.lwsp: 010x00000xxxxx10 rd = 0 -> reserved
        lwsp = ['0b001' + bin(x)[2:].zfill(1) + '00000' + bin(y)[2:].zfill(5) + '10' for y in
                range(0, 2 ** 5) for x in range(0, 2 ** 1)]
        # c.ldsp: 011x00000xxxxx10 rd = 0 -> reserved
        ldsp = ['0b011' + bin(x)[2:].zfill(1) + '00000' + bin(y)[2:].zfill(5) + '10' for y in
                range(0, 2 ** 5) for x in range(0, 2 ** 1)]
