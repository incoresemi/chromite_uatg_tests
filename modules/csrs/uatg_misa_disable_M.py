...
class uatg_misa_disable_mext(IPlugin):
    """
    ...
    def generate_asm(self) -> List[Dict[str, Union[Union[str, list], Any]]]:
        ...
        # Disabling & Enabling M extension
        if 'A' in self.isa and ext.islegal(self.reset_val - (1 << 12)):
            all_m_insts = '\n\t'.join(
                ig.generate_all_m_inst(modifiers=self.modifiers))
            asm_code += f'\ncsrr x3, misa\nla x4, mtrap_sigptr\n' \
                        f'# Disabling & Enabling M extension\n' \
                        f'test_disable_M:{nt}csrr x5, misa{nt}' \
                        f'li x2, {hex(self.reset_val - (1 << 12))}{nt}' \
                        f'csrw misa, x2 # Clear 12th bit{nt}' \
                        f'beq x2, x5, fail_case ' \
                        f'# If the bit is not written{nt}{all_m_insts}{nt}' \
