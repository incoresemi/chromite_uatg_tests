...
class uatg_pmp_ms_xpartial(IPlugin):
    ...
    def execute(self,core_yaml,isa_yaml) -> bool:
        ...
        return helpers.is_pmp_present(isa_yaml) and 'C' in isa_yaml['hart0']['ISA']
    ...
    def generate_asm(self) -> Dict[str, Union[Union[str, list], Any]]:
        ...
        pmp_region = "j start_test;\n.align {0};\npmp_region:\n.rept {1}\nnop;\n.endr\nc.nop;\n.option push\n".format(
                        align, int((self.rsize-4)/4)
                        ) +".option norvc\npmp_target:\nj 1f;\n.option pop\nc.nop;\n"+f"1:\n{sinst} t2,0(t1);\n"+\
                    "jalr x0,ra;\n"
        ...
        for entry in pmps[:-1]:
            ...
            for mode in filter(lambda x: x!= 0, modes):
                label = 'pmp_region'
                test_asm = pmp_region+'start_test:\n'
                test_asm += f'li a0, 173;\nla t1, sig;\n li t2, -1;\n la t3, access_fault;\n'
