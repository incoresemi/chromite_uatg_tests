...
class uatg_mbox_comp_comp_rv64_imm_WAR(IPlugin):
    """
    ...
    def generate_asm(
        ...
        """
         ...
         instructions based upon ISA c.and, c.or, c.xor, c.sub 
         with compressed instructions(rv64-reg-imm) 
         (i.e c.and x3, x1
              c.addiw x1, imm_val)
        """
        # compressed instructions for CI format has no limit to
        # use the registers it will support x0 to x31 registers.
        # Test to validate the mextension instructions with compressed
        # (rv64-reg-imm) instructions.
        ...
        random_list = []
        if 'M' in self.isa or 'Zmmul' in self.isa:
            instruction_list += compressed_instructions[f'{self.isa_bit}-re'
                                                        f'g-regCA']
        if 'i' in self.isa:
            random_list += compressed_instructions[f'{self.isa_bit}-reg-imm']
