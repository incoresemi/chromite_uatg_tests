...
class uatg_mbox_comp_comp_rv64_WAR(IPlugin):
    ...
    def generate_asm(
        ...
        """
         Generates the ASM instructions for compressed 
         instructions with compressed instructions.
         It creates asm for the following instructions based 
         upon ISA c.and, c.or, c.xor, c.sub 
         with compressed instructions(rv64-regCA) 
         (i.e c.or x9, x10
              c.addw x10, x11) 
        """
        # compressed instructions for CA format has limit to
        # use the registers it will support only the x8 to x15.
        # Test to validate the mextension instructions with
        #compressed (rv64-regCA) instructions.
