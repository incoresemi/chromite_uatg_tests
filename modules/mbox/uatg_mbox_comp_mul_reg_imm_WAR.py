...
class uatg_mbox_comp_mul_reg_imm_WAR(IPlugin):
    """
    ...
    def generate_asm(
        ...
        random_list = []
        if 'M' in self.isa or 'Zmmul' in self.isa:
            instruction_list += compressed_instructions[f'{self.isa_bit}-reg-imm']
        if 'i' in self.isa:
