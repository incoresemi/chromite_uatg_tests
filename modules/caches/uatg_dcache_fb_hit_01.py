...
class uatg_dcache_fb_hit_01(IPlugin):
    ...
    def generate_asm(self) -> List[Dict[str, Union[Union[str, list], Any]]]:
        ...
        # high is the largest legal number that can be used as an offset to load/store instructions
        high = 0
        while(high < 2048 - (self._block_size * self._word_size)):
            high = high + (self._block_size * self._word_size)
        
        iter_count = 0 # number of stores should be exactly equal to the size of the fill buffer 
        for i in range(0, high, 64):
            ...
            asm_fb_hit += "\tlw a1, {0}(t2)\n".format(i) # all these loads should lead to a hit in the fill buffer
