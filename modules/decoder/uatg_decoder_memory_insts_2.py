...
class uatg_decoder_memory_insts_2(IPlugin):
    """
            ...
            for rs2 in rs_reg_file:
                ...
                for rs1 in rs_reg_file:
                    for imm_val in imm:
                        ...
                        if 'sh' in inst and adj in [1, 3, 5, 7]:
                            trap_sigbytes = trap_sigbytes + (3 *
                                                             self.offset_inc)
                            trap_count = trap_count + 1
                        elif 'sw' in inst and adj in [1, 2, 3, 5, 6, 7]:
                            trap_sigbytes = trap_sigbytes + (3 *
                                                             self.offset_inc)
                            trap_count = trap_count + 1
                        elif 'sd' in inst and adj in [1, 2, 3, 4, 5, 6, 7]:
                            trap_sigbytes = trap_sigbytes + (3 *
                                                             self.offset_inc)
                            trap_count = trap_count + 1
                        ...
                        if swreg in [rs2, rs1]:
                            newswreg = random.choice([
                                x for x in rs_reg_file
                                if x not in [rs2, rs1, 'x0']
                            ])
                            asm_code += f'mv {newswreg}, {swreg}\n'
                            swreg = newswreg

                        if test_reg in [rs2, rs1, swreg]:
                            newtest_reg = random.choice([
                                x for x in rs_reg_file
                                if x not in [rs2, rs1, 'x0', swreg]
                            ])
                            test_reg = newtest_reg
