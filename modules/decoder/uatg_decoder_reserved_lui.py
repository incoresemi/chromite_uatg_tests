...
class uatg_decoder_illegal_instructions(IPlugin):
    ...
    def generate_asm(self):
        lui = [f'lui x0, {i}' for i in [hex(i) for i in range(0, 2 ** 20)]]

        test_dict = []
        offset = 2 ** 14
        ranges = range(0, len(lui), offset)
        for begin in ranges:
            asm_code = f'.align 4\n\n\n'
            trap_sigbytes = 0
            trap_count = 0
            end = -1 if begin + offset > len(lui) else begin + offset

            for instruction in lui[begin:end]:
